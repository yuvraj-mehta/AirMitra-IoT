...
const History = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      console.log(
        `📊 Fetching history for period: ${timePeriod} from ${timeRange}`
      );
  ...
  const handleExport = () => {
    const now = new Date();
    const dateStamp = format(now, "yyyyMMdd_HHmmss");
    if (activeTab === "sensors") {
      if (!sensorHistory.length) {
        toast({
          title: "No data",
          description: "No sensor data to export for this period.",
          variant: "destructive",
        });
        return;
      }
      const columns = [
        ...
        { key: "time", header: "time_label" },
        { key: "temperature", header: "temperature_c" },
        { key: "humidity", header: "humidity_pct" },
        {
          key: "motion",
          header: "motion_detected",
          map: (r: any) => (r.motion === 100 ? true : false),
        },
      ];
      const csv = objectsToCSV(sensorHistory, columns);
      downloadCSV(csv, `sensors_${timePeriod}_${dateStamp}.csv`);
      toast({ title: "Exported", description: "Sensor data CSV downloaded." });
      return;
    }
    if (activeTab === "devices") {
      if (!deviceHistory.length) {
        toast({
          title: "No data",
          description: "No device history to export for this period.",
          variant: "destructive",
        });
        return;
      }
      const columns = [
        ...
        { key: "time", header: "time_label" },
        {
          key: "bulbState",
          header: "bulb_state",
          map: (r: any) => (r.bulbState === 100 ? "ON" : "OFF"),
        },
        {
          key: "fanState",
          header: "fan_state",
          map: (r: any) => (r.fanState === 100 ? "ON" : "OFF"),
        },
        { key: "fanSpeed", header: "fan_speed_pct" },
        { key: "rgbColor", header: "rgb_color" },
        { key: "red", header: "red" },
        { key: "green", header: "green" },
        { key: "blue", header: "blue" },
      ];
      const csv = objectsToCSV(deviceHistory, columns);
      downloadCSV(csv, `devices_${timePeriod}_${dateStamp}.csv`);
      toast({
        title: "Exported",
        description: "Device history CSV downloaded.",
      });
      return;
    }
    if (activeTab === "events") {
      if (!events.length) {
        toast({
          title: "No data",
          description: "No events to export for this period.",
          variant: "destructive",
        });
        return;
      }
      ...
      const csv = objectsToCSV(events, columns as any);
      downloadCSV(csv, `events_${timePeriod}_${dateStamp}.csv`);
      toast({ title: "Exported", description: "Events CSV downloaded." });
    }
  };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <TabsContent value="sensors" className="mt-6">
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-semibold mb-4">
              Temperature, Humidity & Motion ({timePeriod})
            </h3>
        ...
        <TabsContent value="devices" className="mt-6">
          <Card className="p-6 bg-card border-border">
            <h3 className="text-lg font-semibold mb-4">
              Device Status History ({timePeriod})
            </h3>
            <ResponsiveContainer width="100%" height={400}>
          ...
          <Card className="p-6 bg-card border-border mt-6">
            <h3 className="text-lg font-semibold mb-4">
              RGB Color Components ({timePeriod})
            </h3>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={deviceHistory}>
                <CartesianGrid
